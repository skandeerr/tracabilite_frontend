...
import { deleteClientById, getClientById, getClients } from '../../api/clients';
import { useRouter } from 'next/navigation';
import Swal from 'sweetalert2';



const col = ['id', 'fullName', 'adresse', 'numero', 'email'];
...
const Export = () => {
    ...
    useEffect(() => {
        setInitialRecords(() => {
            return rowData.filter((item: any) => {
                return (
                    item.prenom.toString().includes(search.toLowerCase()) ||
    ...
    const header = ['Id', 'fullName', 'adresse', 'numero', 'email'];
    ...
    const deleteRow = async (id: any) => {
        if (id) {
          try {
            await deleteClientById(id ).then((response) => {
  ...
  <button className="btn btn-primary text-white px-2 py-1 rounded-md" type='button' onClick={()=>router.push("/dashboard/effectifs/clients/add")}> <span className='text-sm'>Ajouter</span></button>
</div>
                ...
                <div className="datatables">
                    <DataTable
                        ...
                        columns={[
                            ...
                            { accessor: 'Nom', sortable: true,render: ({ nom }) => <div>{nom}</div> },
                            { accessor: 'Prénom', sortable: true,render: ({ prenom }) => <div>{prenom}</div> },
                            { accessor: 'Adresse', sortable: true,render: ({ adresse }) => <div>{adresse}</div> },
                            { accessor: 'Numéro', title: 'Company', sortable: true,render: ({ numero }) => <div>{numero}</div> },
                            { accessor: 'Email', title: 'Age', sortable: true,render: ({ email }) => <div>{email}</div> },
                            { accessor: 'Action', title: 'Action', sortable: true,render: ({ id }) => <div className="mx-auto ml-3 flex w-max items-center gap-4">
                                <button  className="flex hover:text-info" type='button' onClick={()=>router.push("/dashboard/effectifs/clients/edit/"+id)}>
                        ...
                        <button type="button" className="flex hover:text-danger" onClick={()=>{setShowModal(true);getClient(id)}} >
                    ...
                    <div className="mt-2 text-center sm:ml-4 sm:text-left px-6">
                      ...
                      <div className="p-6 text-center">
                        ...
                        <h4 className="mb-5 font-normal text-gray-500 dark:text-gray-400">
                          Êtes-vous sûr(e) de vouloir supprimer cette Client ?
                        </h4>
                        <button
                          onClick={(e) => deleteRow(client.id)}
                          data-modal-hide="popup-modal"
                          type="button"
